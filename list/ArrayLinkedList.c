...
static void DeletedIndex(List *list, Index idx)
{
    if(list->deleted == Null){
        list->deleted = idx;
...
int Initialize(List *list, int size)
{
...
void Print(const List *list)
{
    if(list->head == Null)
        ...
        puts("---");
        while(ptr != Null){
            PrintLnCurrent(&list->n[ptr].data);
            ptr = list->n[ptr].next;
        }
    }
}
